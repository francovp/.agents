...
8. Never create duplicate Jira issues or duplicate PRs.
...
10. Use the GitHub issue number as the dedupe key for Jira.
11. Prefer live repo state over assumptions.
12. Prefer `gh` and `acli jira` CLIs over MCP tools when available.
13. Distinguish local blockers from global blockers.
14. Stop cleanly on global blockers, ambiguity, or missing ownership.
15. **GitHub auth fallback**: Start with the currently authenticated `gh` account. If it has no permissions for the current repository, try another logged-in `gh` user with `gh auth switch` and restore the original user when the workflow completes.
...
17. When creating or updating a Jira issue, write a readable ticket body with `Summary`, `Context`, `Acceptance Criteria`, and `References` sections so the tracker stays self-contained.
18. **Always include the Jira issue ID (e.g., `SREF-01`) in the PR title in parentheses at the end.** Example: `feat: my awesome feature (SREF-01)`. Also include the Jira issue ID in the description/body of both the GitHub issue and the PR.
...
1. **Verify gh authentication and repo access** — ensure `gh` is authenticated, then validate repository access with fallback to another logged-in user when needed:
   ```bash
   source scripts/gh-auth-utils.sh && trap 'restore_gh_user' EXIT && ensure_repo_access_with_fallback
   ```
...
6. For the primary issue:
   - Check any linked or related Jira issue.
   ...
   - **Check if any linked PR is already merged**: If a PR that references this issue was already merged into `master`/`main`, clean up the `agent-working` label if present (issue + PR), sync Jira to `Shipped`, and end with outcome `SHIPPED`.
   ...
   - **Extract any existing Jira ID** from the issue body (scan for patterns like `SREF-XX` or `(SREF-XX)`) and store it as `JIRA_ISSUE_ID` for use in later steps. If no ID is found, set `JIRA_ISSUE_ID=""`.
...
2. If no Jira issue exists:
   ...
   - Link the Jira issue back to the GitHub issue.
   - **Capture the Jira issue ID** from the creation response (e.g., `SREF-42`) and store it as `JIRA_ISSUE_ID`.
   - **Update the GitHub issue description** to include the Jira ID: append a `---` separator and a `**Jira**: [SREF-XX](https://<jira-instance>.atlassian.net/browse/...)` reference line at the bottom of the issue body. This makes the connection visible in the GitHub UI without editing the original issue content.
   - Add `agent-working` label to the GitHub issue (if not already present).

3. If a Jira issue exists:
   ...
   - If multiple Jira issues remain ambiguous, end with `AMBIGUOUS`.
   - **Extract the Jira issue ID** from the existing issue and store it as `JIRA_ISSUE_ID`.
   - **Ensure the GitHub issue description** includes the Jira ID reference. If missing, add `**Jira**: [SREF-XX](...)` to the issue body.
...
5. **Create the context file** `context/<git-branch-name>.md` with the branch summary:
   ...
   - **References section**: Include `**Jira**: [SREF-XX](https://<jira-instance>.atlassian.net/browse/...)` with the actual issue URL.
...
6. If the PR is ready to land and the agent is confident no human review is needed:
   ...
   - **Remove `agent-working` from the issue and the PR**:
     ...
     ```
   - Sync GitHub/Jira to the shipped state.
...
1. If `LOCAL_DEADLOCK`: Write a concise blocker summary on the issue or PR. Sync GitHub, Jira, and PR states.
2. **If the issue has a merged PR**: Clean up stale `agent-working` labels (issue + PR), sync Jira to `Shipped`, and end with outcome `SHIPPED` (same handling as Step 1).
3. If `IN_REVIEW` with no agent writes: Do not modify the issue, PR, or Jira state — everything is already correct.
...
4. Add the `In review` label to the GitHub issue and PR:
   ...
   ```
5. Move the Jira issue to the `In review` column.
...
- **GitHub CLI Authentication Failures**: If `gh` CLI calls fail due to auth:
  - Verify authentication with `gh auth status`.
  - If not authenticated, suggest the user to run `gh auth login` and wait for user confirmation.
  - If authenticated but access is denied for the current repo, run `ensure_repo_access_with_fallback` to try another logged-in user.
  - If fallback switching succeeds, continue and ensure `restore_gh_user` runs at exit.
  - If auth or fallback still fails, check if `GITHUB_TOKEN` env var is overriding keyring-based auth.
  - As last resort, check if `GITHUB_TOKEN` env var is loaded. If CLI is unavailable, fallback to MCP commands. If both fail:
